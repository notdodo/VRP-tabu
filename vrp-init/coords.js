...
$(function() {
  ...
  for (var n in pars) pars[n].disabled = false;
  // open a file
...
function saveToFile() {
  ...
  } else alert("Please insert some customers");
}
...
function parseCustomer(cs) {
  ...
  if (Array.isArray(cs.vertices)) {
    ...
    for (var c in cs.vertices) {
      ...
      if (parseInt(i, 10) < 10) a.name = "V0" + i;
      else a.name = "V" + i;
      var id = "<span class='num'>" + "V<sub>" + i + "</sub>" + "</span> ";
      var app =
        id +
        $.strPad(cs.vertices[c].x, 4) +
        "; " +
        $.strPad(cs.vertices[c].y, 4) +
        "<br/>";
    ...
    for (var n in pars) pars[n].disabled = true;
...
function onMouse(e) {
  var id = -1;
  if (intRegex.test(e.target.uid)) id = e.target.uid.match(intRegex)[0];
  if (id >= 0) {
...
function drawOneRoute(r) {
  ...
  if (lastRemoved === null) lastRemoved = route;
  else {
    route.draw = true;
    canvas.add(route);
  }
...
function drawAll(r) {
  for (var route in routes) {
    if (!routes[route].draw && route != r) canvas.add(routes[route].path);
  }
}
  ...
  while (data[i].lastIndexOf("EOF") < 0) {
    ...
    if (data[i].lastIndexOf("NODE_COORD_SECTION") >= 0 && dimension > 0) {
      ...
      while (iCustomer < dimension) {
        ...
        if (iCustomer + 1 < 10) var n = "V0" + (iCustomer + 1);
        else var n = "V" + (iCustomer + 1);
        item = {
          name: n,
          x: X,
          y: Y,
